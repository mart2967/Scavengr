				...
				<div class="row-fluid prompt-row" >
					<div class="span4">
						<g:if test="${promptPhotoContainer[1]}">
							<g:link controller="prompt" action="show" mapping="prompt" params="[key:huntInstance?.key]"  id="${promptPhotoContainer[0].id}" class="btn btn-prompt btn-success">
							<g:fieldValue bean="${promptPhotoContainer[0]}" field="title" />
							<i class="icon-ok"></i>
							</g:link>
						</g:if>
						<g:else>
							<g:link controller="prompt" action="show" mapping="prompt" params="[key:huntInstance?.key]" id="${promptPhotoContainer[0].id}" class="btn btn-prompt btn-primary">
							<g:fieldValue bean="${promptPhotoContainer[0]}" field="title" />
							</g:link>
						</g:else>
					</div>
			...
			<div class="well">
				<ul class="nav nav-list">
					...
					<g:if test="${huntInstance?.startDate}">
							...
							<dd>
								<g:formatDate date="${huntInstance?.startDate}" type="datetime" dateStyle="LONG"/>
							</dd>
					...
					<g:if test="${huntInstance?.endDate}">
							...
							<dd>
								<g:formatDate date="${huntInstance?.endDate}" type="datetime" dateStyle="LONG"/>
							</dd>
					...
					<g:if test="${huntInstance?.myUsers}">
						...
						<g:each in="${huntInstance?.myUsers}" var="user">
							...
							<g:if test="${isCreatorOrAdmin}" >
								<g:link class="pull-right" style="text-decoration:none;" action="removeUser" params="['login':user?.login, 'myHunt.id':huntInstance?.id]"><i class="icon-ban-circle"></i></g:link>
							</g:if>
							</dd>
						</g:each>
						</g:if>
					...
					<g:if test="${isCreatorOrAdmin}" >
						...
						<g:textField name="title" placeholder="Title"/><br />
						<g:textArea name="description" placeholder="Description" />
						<g:hiddenField name="myHunt.id" value="${huntInstance.id}"/>
						<button id="new-prompt-submit" type="submit" class="btn btn-primary"><i class="icon-plus icon-white"></i>Add Prompt</button>
						</g:form>

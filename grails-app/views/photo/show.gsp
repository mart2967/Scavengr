...
<html>
	<head>
		<link rel="stylesheet" href="${resource(dir: 'css', file: 'bootstrap-lightbox.less')}" type="text/css">
		<script type="text/javascript" src="${resource(dir: 'js', file: 'bootstrap-lightbox.js')}"></script>
		...
		<div class="row-fluid">
			...
			<div class="span9">
				...
				<div class="page-header">
					<h1>${photoInstance?.title}</h1>
					<g:if test="${photoInstance?.description}">
						<h5><g:fieldValue bean="${photoInstance}" field="description"/></h5>
					</g:if>
...
<%--					</g:if>--%>
				...
				<div id="photoLightbox" class="lightbox hide fade"  tabindex="-1" role="dialog" aria-hidden="true">
				    <div class='lightbox-header'>
				        <button type="button" class="close" data-dismiss="lightbox" aria-hidden="true">&times;</button>
				    </div>
				    <div class='lightbox-content'>
				        <img src="${createLink(controller:'photo', action:'viewImage', id: photoInstance?.id, params:[size:original])}">
				    </div>
...
<%--					        <button class="btn btn-small btn-primary">Click</button>--%>
				...
				<ul class="pager">
					<g:if test="${prevId}">
						<li class="previous">
							<g:link action="show" id="${prevId}" elementId="previousPicture">
							&larr; Previous
							</g:link>
						</li>
					</g:if>
					...
					<ul class="nav nav-list">
						...
						<g:if test="${photoInstance?.myPrompt}">
							<g:if test="${!photoInstance?.myPrompt?.myHunt?.isPrivate}" >
								...
								<dd>
									<g:link controller="prompt" action="show" mapping="prompt" params="[key:photoInstance?.myPrompt.myHunt?.key, id: photoInstance?.myPrompt.id]" > 

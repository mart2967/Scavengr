...
class HuntController {
	...
	def list() {
		if (session.hunter != null) {
			redirect action: 'show', params: [key: session.key]
		} else {
	...
	def invite() {
		if (session.hunter != null) {
			...
			def link = grailsLinkGenerator.link(
					controller: 'hunt', action: 'show', params: [key: huntInstance.key])
			if(GenericValidator.isEmail(user)){
				...
				if(userInstance != null){
					...
					NotifierService.sendNotification(huntInstance.myCreator, userInstance, huntInvitiation,
							invited + quote + huntInstance.title + quote, link, goToHunt)
				}
		...
		if (session.hunter != null) {
			...
			switch (request.method) {
				...
				case getPost[1]:
					...
					if (!huntInstance.save([flush: true])) {
						redirect action: 'edit', params:params
						return
					}
	...
	def delete() {
		if (session.hunter != null) {
			...
			try {
				...
				huntInstance.delete([flush: true])

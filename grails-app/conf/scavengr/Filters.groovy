            ...
            after = { model ->
                try{
                    ...
                    if(user != null && session.participant == null){
                        model?.messages = Notification.findAll("from Notification as n where n.recipient=:user and n.isRead=:f",
                            [user:user,f:false],[sort:'dateCreated', order:'desc'])
                        model?.numMessages = Notification.findAll("from Notification as n where n.recipient=:user and n.isRead=:f",
